...
def GenerateApplicationFirmware(srcfile, destfile, srcfldrfile, destfldrfile, swver, hwver, partnum, addversioninfo = False):
    if not os.path.exists(os.path.split(destfile)[0]):
        os.makedirs(os.path.split(destfile)[0])

    try:
        # 处理flash driver
        if os.path.exists(destfldrfile):
            os.remove(destfldrfile)
        shutil.copy(srcfldrfile, destfldrfile)

        newflashdriverfile = []
        with open(destfldrfile, 'r', encoding='utf-8') as fdf:
            newflashdriverfile = fdf.readlines()
            
        partnumber = binascii.b2a_hex(partnum.encode('utf-8')).decode('utf-8')
        ...
        with open(destfldrfile, 'w', encoding='utf-8') as fdf:
        ...
        apps0line += binascii.b2a_hex(swver.encode('utf-8')).decode('utf-8')
...
if __name__ == "__main__":
    ...
    pn = info["part number"]
    GenerateApplicationFirmware(SRC_FILE, DEST_FILE, SRC_FLASH_DRIVER_FILE, DEST_FLASH_DRIVER_FILE, swv, hwv, pn, addversioninfo = True)
